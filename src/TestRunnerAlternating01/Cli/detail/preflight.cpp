...
namespace crsce::testrunner_alternating01::cli {
    ...
    void preflight_check_alternating01(const std::filesystem::path &in_path,
        ...
        // Helper: compute linear index r*S + c without lambdas (ODPCPP-friendly)
        struct Indexer final {
            static std::size_t idx(std::size_t r, std::size_t c, std::size_t S_) noexcept {
                return (r * S_) + c;
            }
        };
        ...
        for (std::size_t i = 0; i < S; ++i) {
            if (b.at(Indexer::idx(i, i, S)) != 0) {
                throw std::runtime_error("preflight: CSM[i,i] not zero at i=" + std::to_string(i));
            }
        }
        // 2) CSM[0,1] == 1
        if (b.at(Indexer::idx(0, 1, S)) != 1) { throw std::runtime_error("preflight: CSM[0,1] != 1"); }
        // 3) CSM[1,0] == 1
        if (b.at(Indexer::idx(1, 0, S)) != 1) { throw std::runtime_error("preflight: CSM[1,0] != 1"); }
        // 4) CSM[510,509] == 1
        if (b.at(Indexer::idx(S - 1, S - 2, S)) != 1) { throw std::runtime_error("preflight: CSM[510,509] != 1"); }
        // 5) CSM[509,510] == 1
        if (b.at(Indexer::idx(S - 2, S - 1, S)) != 1) { throw std::runtime_error("preflight: CSM[509,510] != 1"); }
        // 6) CSM[0,510] == CSM[510,0]
        if (b.at(Indexer::idx(0, S - 1, S)) != b.at(Indexer::idx(S - 1, 0, S))) {
        ...
        for (std::size_t d = 0; d < S; ++d) {
            ...
            for (std::size_t r = 0; r < S; ++r) {
                ...
                sum_d += static_cast<std::size_t>(b.at(Indexer::idx(r, c, S)));
            }
        ...
        for (std::size_t x = 0; x < S; ++x) {
            ...
            for (std::size_t r = 0; r < S; ++r) {
                ...
                sum_x += static_cast<std::size_t>(b.at(Indexer::idx(r, c, S)));
            }
