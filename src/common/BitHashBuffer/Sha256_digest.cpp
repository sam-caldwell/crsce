...
std::array<u8, 32> sha256_digest(const u8* data, std::size_t len) {
  ...
  for (std::size_t chunk = 0; chunk < full_chunks; ++chunk) {
    ...
    for (int i = 16; i < 64; ++i) {
      ...
      w.at(i_sz) = small_sigma1(w.at(i_sz - 2)) + w.at(i_sz - 7) + small_sigma0(w.at(i_sz - 15)) + w.at(i_sz - 16);
    }
  ...
  if (rem + 1 + 8 <= 64) {
    ...
    for (int i = 0; i < 16; ++i) {
      w.at(static_cast<std::size_t>(i)) = load_be32(tail_span.subspan(static_cast<std::size_t>(i) * 4U).data());
    }
    for (int i = 16; i < 64; ++i) {
      const auto i_sz = static_cast<std::size_t>(i);
      w.at(i_sz) = small_sigma1(w.at(i_sz - 2)) + w.at(i_sz - 7) + small_sigma0(w.at(i_sz - 15)) + w.at(i_sz - 16);
    ...
    for (int i = 0; i < 16; ++i) {
      w.at(static_cast<std::size_t>(i)) = load_be32(tail_span2.subspan(static_cast<std::size_t>(i) * 4U).data());
    }
    for (int i = 16; i < 64; ++i) {
      const auto i_sz = static_cast<std::size_t>(i);
      w.at(i_sz) = small_sigma1(w.at(i_sz - 2)) + w.at(i_sz - 7) + small_sigma0(w.at(i_sz - 15)) + w.at(i_sz - 16);
    ...
    for (int i = 0; i < 16; ++i) {
      w.at(static_cast<std::size_t>(i)) = load_be32(tail2_span.subspan(static_cast<std::size_t>(i) * 4U).data());
    }
    for (int i = 16; i < 64; ++i) {
      const auto i_sz = static_cast<std::size_t>(i);
      w.at(i_sz) = small_sigma1(w.at(i_sz - 2)) + w.at(i_sz - 7) + small_sigma0(w.at(i_sz - 15)) + w.at(i_sz - 16);
