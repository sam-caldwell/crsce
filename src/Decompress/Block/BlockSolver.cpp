...
namespace {
    ...
    // Enforce that each row's ones count equals LSM[r].
    // Used to guard phases after BitSplash so row_avg_pct stays 100.
...
namespace crsce::decompress {
    ...
    bool solve_block(const std::span<const std::uint8_t> lh, // NOLINT(readability-function-size)
        ...
        if (constexpr std::uint64_t total_bits = static_cast<std::uint64_t>(S) * static_cast<std::uint64_t>(S);
            ...
            // Optional debug: emit a one-line summary when enabled
        ...
        // Seed initial beliefs from LSM, balanced by VSM/DSM/XSM residual pressure and add a small jitter (±0.02)
        std::uint64_t belief_seed_used = 0;
        {
            ...
            for (std::size_t r = 0; r < S; ++r) {
                ...
                for (std::size_t c = 0; c < S; ++c) {
                    ...
                    const std::size_t d = ::crsce::decompress::detail::calc_d(r, c);
                    const std::size_t x = ::crsce::decompress::detail::calc_x(r, c);
                    const double col_need = static_cast<double>(st.R_col.at(c)) / static_cast<double>(std::max<
                                                std::uint16_t>(1, st.U_col.at(c)));
                    const double diag_need = static_cast<double>(st.R_diag.at(d)) / static_cast<double>(std::max<
                                                 std::uint16_t>(1, st.U_diag.at(d)));
                    const double x_need = static_cast<double>(st.R_xdiag.at(x)) / static_cast<double>(std::max<
                                              std::uint16_t>(1, st.U_xdiag.at(x)));
                ...
                // Mark chosen columns as high belief; the rest as low
        ...
        // Run staged phases (BitSplash, then Radditz Sift) before GOBP fallback.
        snap.phase = BlockSolveSnapshot::Phase::rowPhase;
        ::crsce::o11y::event("bitsplash_start");
        snap.bitsplash_status = 1; // started
        const std::size_t rp = ::crsce::decompress::phases::bit_splash(csm_out, st, snap, 0);
        if (rp > 0) {
        ...
        // Determine BitSplash completion: every row has exactly LSM[r] ones in CSM
        bool bitsplash_complete = true;
        for (std::size_t r = 0; r < S; ++r) {
            std::size_t ones = 0;
            for (std::size_t c = 0; c < S; ++c) { if (csm_out.get(r, c)) { ++ones; } }
            if (ones != static_cast<std::size_t>(lsm.at(r))) {
                bitsplash_complete = false;
                break;
            }
        }
        snap.bitsplash_status = bitsplash_complete ? 3 : 2;
        if (!bitsplash_complete) {
            // Emit diagnostics to stderr with a concise mismatch summary
            try {
                // Gather first/last few mismatches
                std::vector<std::size_t> bad;
                bad.reserve(16);
                for (std::size_t r = 0; r < S; ++r) {
                    // compute ones in row r
                    std::size_t ones = 0;
                    for (std::size_t c = 0; c < S; ++c) {
                        if (csm_out.get(r, c)) { ++ones; }
                    }
                    if (ones != static_cast<std::size_t>(lsm.at(r))) {
                        bad.push_back(r);
                ...
                for (std::size_t i = 0; i < bad.size(); ++i) {
                    ...
                    std::size_t ones = 0;
                    for (std::size_t c = 0; c < S; ++c) { if (csm_out.get(r, c)) { ++ones; } }
                    sample += "(r=" + std::to_string(r) + ",have=" + std::to_string(ones) + ",lsm=" + std::to_string(
                        lsm.at(r)) + ")";
                    if (i + 1U < bad.size()) { sample += ", "; }
                }
                std::print(stderr,
                           "bitsplash: failed rows={} sample={} (seed_belief={})\n",
                           static_cast<std::uint64_t>(bad.size()), sample,
                           static_cast<std::uint64_t>(snap.rng_seed_belief));
            } catch (...) {
                /* ignore diagnostics failures */
            }
            // Hard fail BitSplash per policy
            snap.message = "bitsplash rows not satisfied";
            set_block_solve_snapshot(snap);
            return false;
        }

        // Run Radditz only if BitSplash completed
        if (bitsplash_complete) {
            ...
            (void)rsf; // Do not call DE here; R/U not updated by Radditz per policy
            snap.U_row.assign(st.U_row.begin(), st.U_row.end());
            snap.U_col.assign(st.U_col.begin(), st.U_col.end());
            snap.U_diag.assign(st.U_diag.begin(), st.U_diag.end());
            snap.U_xdiag.assign(st.U_xdiag.begin(), st.U_xdiag.end());
            ...
            // Radditz completion: columns match VSM and rows remain equal to LSM
            bool radditz_complete = true;
            for (std::size_t c = 0; c < S; ++c) {
                std::size_t ones = 0;
                for (std::size_t r = 0; r < S; ++r) { if (csm_out.get(r, c)) { ++ones; } }
                if (ones != static_cast<std::size_t>(vsm.at(c))) {
                    radditz_complete = false;
                    break;
                }
            }
            if (radditz_complete && !rows_match_lsm(csm_out, lsm)) {
                radditz_complete = false;
                snap.message = "radditz violated row-sum invariant";
            }
            snap.radditz_status = radditz_complete ? 3 : 2;
            if (!radditz_complete) {
                try {
                    // Emit diagnostics to stderr with concise mismatch summary
                    std::vector<std::size_t> bad;
                    bad.reserve(16);
                    for (std::size_t c = 0; c < S; ++c) {
                        std::size_t ones = 0;
                        for (std::size_t r = 0; r < S; ++r) { if (csm_out.get(r, c)) { ++ones; } }
                        if (ones != static_cast<std::size_t>(vsm.at(c))) { bad.push_back(c); if (bad.size() >= 16) { break; } }
                    }
                    std::string sample;
                    for (std::size_t i = 0; i < bad.size(); ++i) {
                        ...
                        std::size_t ones = 0;
                        for (std::size_t r = 0; r < S; ++r) { if (csm_out.get(r, c)) { ++ones; } }
                        sample += "(c=" + std::to_string(c) + ",have=" + std::to_string(ones) + ",vsm=" + std::to_string(vsm.at(c)) + ")";
                        if (i + 1U < bad.size()) { sample += ", "; }
                    }
                    std::print(stderr,
                               "radditz: failed cols={} sample={} (seed_belief={})\n",
                               static_cast<std::uint64_t>(bad.size()), sample,
                               static_cast<std::uint64_t>(snap.rng_seed_belief));
            ...
            for (int rs = 0; rs <= kRestarts && !det.solved(); ++rs) {
                ...
                if (multiphase) {
                    ...
                    if (par_rs > 1 && rs < kRestarts) {
                        ...
                        const std::size_t base_valid = ::crsce::decompress::detail::compute_prefix(
                            pc_ver_seen, pc_ok, pc_prefix_len, csm_out, st, lh, snap);
                        ...
                        const std::size_t tasks = std::min<std::size_t>(
                            workers, static_cast<std::size_t>(kRestarts - rs));
                        ...
                        for (const auto &arr: ev_phase) {
                            for (const auto &ep: arr) { if (!ep.name.empty()) { snap.thread_events.push_back(ep); } }
                        }
                        if (adopted.load(std::memory_order_acquire)) {
                            ...
                            ev.restart_index = rs + 1;
                            ev.prefix_rows = base_valid + 1;
                            ev.unknown_total = snap.unknown_total;
                            ev.action = BlockSolveSnapshot::RestartAction::lockInParRs;
                    ...
                    while (::crsce::decompress::detail::commit_any_verified_rows(
                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs)) {
                    ...
                    (void) ::crsce::decompress::detail::finish_near_complete_any_row(
                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                    int since_restart = 0;
                    const int dynamic_cooldown_base = 20;
                    // aggressively reduced cooldown for earlier contradiction restarts
                    ...
                    for (std::size_t ph = 0; ph < dampers.size() && !det.solved(); ++ph) {
                        ...
                        {
                            std::size_t sumU0 = 0;
                            for (const auto u: st.U_row) { sumU0 += static_cast<std::size_t>(u); }
                            ::crsce::o11y::metric(
                                "gobp_phase_start",
                                1LL,
                                {
                        ...
                        for (int gi = 0; gi < iters_arr.at(ph); ++gi) {
                            ...
                            try {
                                snap.phase = BlockSolveSnapshot::Phase::gobp;
                                {
                                    ...
                                    snap.time_gobp_ms += static_cast<std::size_t>(std::chrono::duration_cast<
                                        std::chrono::milliseconds>(t1g - t0g).count());
                                    gprog += gobp_only;
                                ...
                                // Follow with a DE sweep to propagate any newly forced moves
                                {
                                    ...
                                    gprog += vv;
                                    const auto ms = static_cast<std::size_t>(std::chrono::duration_cast<
                                        std::chrono::milliseconds>(t1d - t0d).count());
                                    snap.time_de_ms += ms;
                            ...
                            snap.U_xdiag.assign(st.U_xdiag.begin(), st.U_xdiag.end());
                            {
                                ...
                                if (trace_sumu_enabled()) {
                                    ::crsce::o11y::metric(
                                        ...
                                        {
                                            {"phase", std::to_string(ph)}, {"sumU", std::to_string(sumU)},
                                ...
                                if ((gi % heartbeat) == 0) {
                                    ...
                                    if (gi_span >= roi_min_iters) {
                                        const double imp =
                                                static_cast<double>(roi_last_sumU) - static_cast<double>(sumU);
                                        const double per1k = (gi_span > 0)
                                                                 ? (imp * 1000.0 / static_cast<double>(gi_span))
                                                                 : imp * 1000.0;
                            ...
                            try {
                                bool any_commit = false;
                                {
                                    ...
                                    snap.time_lh_ms += static_cast<std::size_t>(std::chrono::duration_cast<
                                        std::chrono::milliseconds>(t1v - t0v).count());
                                }
                                if (any_commit) {
                            ...
                            // Per-row/col gating and finishers with sink-avoidance
                            {
                                ...
                                if (do_verify) {
                                    ...
                                    const bool any1 = ::crsce::decompress::detail::commit_valid_prefix(
                                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                                    const auto t1v1 = std::chrono::steady_clock::now();
                                    snap.time_lh_ms += static_cast<std::size_t>(std::chrono::duration_cast<
                                        std::chrono::milliseconds>(t1v1 - t0v1).count());
                                    if (any1) {
                                        any_commit = true;
                                    }
                                    const auto t0v2 = std::chrono::steady_clock::now();
                                    const bool any2 = ::crsce::decompress::detail::commit_any_verified_rows(
                                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                                    const auto t1v2 = std::chrono::steady_clock::now();
                                    snap.time_lh_ms += static_cast<std::size_t>(std::chrono::duration_cast<
                                        std::chrono::milliseconds>(t1v2 - t0v2).count());
                                    if (any2) {
                                        any_commit = true;
                                    }
                                }
                                {
                                    const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                    const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                    const bool ok_ad = ::crsce::decompress::detail::finish_boundary_row_adaptive(
                                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs, since_restart);
                                    if (ok_ad && !rows_match_lsm(csm_out, lsm)) {
                                        csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst;
                                        ++snap.verify_rows_failures;
                                    }
                                }
                                    ...
                                    {
                                        {
                                            const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                            const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                            const bool ok = ::crsce::decompress::detail::finish_boundary_row_micro_solver(
                                                csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                                            if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                        }
                                        {
                                            const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                            const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                            const bool ok = ::crsce::decompress::detail::finish_boundary_row_segment_solver(
                                                csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                                            if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                        }
                                        {
                                            const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                            const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                            const bool ok = ::crsce::decompress::detail::finish_two_row_micro_solver(
                                                csm_out, st, lh, baseline_csm, baseline_st, snap, rs);
                                            if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                        }
                                    }
                                ...
                                std::vector<std::pair<double, std::size_t> > row_scores;
                                row_scores.reserve(S);
                                std::vector<std::pair<double, std::size_t> > col_scores;
                                col_scores.reserve(S);
                                const double w1 = 0.6;
                                const double w2 = 0.4;
                                ...
                                std::ranges::sort(row_scores, std::greater<double>{},
                                                  &std::pair<double, std::size_t>::first);
                                std::ranges::sort(col_scores, std::greater<double>{},
                                                  &std::pair<double, std::size_t>::first);
                                std::vector<std::size_t> best_rows;
                                best_rows.reserve(24);
                                std::vector<std::size_t> best_cols;
                                best_cols.reserve(16);
                                ...
                                const bool r_try = [&]{
                                    const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                    const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                    const bool ok = ::crsce::decompress::detail::finish_near_complete_rows_scored(
                                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs, best_rows, 256);
                                    if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; return false; }
                                    return ok;
                                }();
                                const bool c_try = [&]{
                                    const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                    const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                    const bool ok = ::crsce::decompress::detail::finish_near_complete_columns_scored(
                                        csm_out, st, lh, baseline_csm, baseline_st, snap, rs, best_cols, 160);
                                    if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; return false; }
                                    return ok;
                                }();
                                if (!r_try) {
                                    const std::size_t rows_top_n = std::min<std::size_t>(
                                        64, 16 + static_cast<std::size_t>(since_restart / 200));
                                    {
                                        const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                        const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                        const bool ok = ::crsce::decompress::detail::finish_near_complete_top_rows(
                                            csm_out, st, lh, baseline_csm, baseline_st, snap, rs, rows_top_n, 256);
                                        if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                    }
                                }
                                if (!c_try) {
                                    const std::size_t cols_top_n = std::min<std::size_t>(
                                        32, 8 + static_cast<std::size_t>(since_restart / 300));
                                    {
                                        const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                        const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                        const bool ok = ::crsce::decompress::detail::finish_near_complete_top_columns(
                                            csm_out, st, lh, baseline_csm, baseline_st, snap, rs, cols_top_n, 160);
                                        if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                    }
                                }
                                if (any_commit) {
                                ...
                                if (gprog == 0 && plateau_flip_attempts >= 3) {
                                    ...
                                    if (do_verify2) {
                                        ...
                                        snap.time_lh_ms += static_cast<std::size_t>(std::chrono::duration_cast<
                                            std::chrono::milliseconds>(t1v3 - t0v3).count());
                                    }

                                    {
                                        ...
                                        const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                        const bool ok = ::crsce::decompress::detail::finish_near_complete_top_rows(
                                            csm_out, st, lh, baseline_csm, baseline_st, snap, rs, 32, 256);
                                        if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                    }
                                    {
                                        const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                        const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                        const bool ok = ::crsce::decompress::detail::finish_near_complete_top_columns(
                                            csm_out, st, lh, baseline_csm, baseline_st, snap, rs, 16, 160);
                                        if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                    }
                                    {
                                        const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                        const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                        const bool ok = ::crsce::decompress::detail::finish_boundary_row_adaptive(
                                            csm_out, st, lh, baseline_csm, baseline_st, snap, rs, since_restart);
                                        if (ok && !rows_match_lsm(csm_out, lsm)) { csm_out = pre_csm; st = pre_st; baseline_csm = pre_base; baseline_st = pre_bst; ++snap.verify_rows_failures; }
                                    }
                                    ...
                                    if (const bool on_polish3 = ((ph + 1U) == dampers.size());
                                        !only_polish3 || on_polish3) {
                                        {
                                            const Csm pre_csm = csm_out; const ConstraintState pre_st = st;
                                            const Csm pre_base = baseline_csm; const ConstraintState pre_bst = baseline_st;
                                            const bool ok = ::crsce::decompress::detail::finish_boundary_row_micro_solver(
                                ...
                                if (!snap.restarts.empty()) {
                                    ...
                                    int cnt = 0;
                                    for (const auto r0: recent_restart_rows) {
                                        if (r0 == last_r) {
                                            ++cnt;
                                        }
                                    }
                            ...
                            // to tip ≥95% rows to 100% and grow the verified prefix.
                            {
                                ...
                                if (boundary < S) {
                                    const std::size_t valid_now = ::crsce::decompress::detail::compute_prefix(
                                        pc_ver_seen, pc_ok, pc_prefix_len, csm_out, st, lh, snap);
                                    ...
                                    if (st.U_row.at(boundary) > 0 && st.U_row.at(boundary) <= near_thresh) {
                                        ...
                                        while (st.U_row.at(boundary) > 0 && steps < kFocusMaxSteps) {
                                            ...
                                            for (std::size_t c0 = 0; c0 < S; ++c0) {
                                                ...
                                                const std::size_t d0 =
                                                        ::crsce::decompress::detail::calc_d(boundary, c0);
                                                const std::size_t x0 =
                                                        ::crsce::decompress::detail::calc_x(boundary, c0);
                                                ...
                                                const double u_x =
                                                        std::max(1.0, static_cast<double>(st.U_xdiag.at(x0)));
                                                const double rneed_c = std::fabs(
                                                    (static_cast<double>(st.R_col.at(c0)) / u_c) - 0.5);
                                                const double rneed_d = std::fabs(
                                                    (static_cast<double>(st.R_diag.at(d0)) / u_d) - 0.5);
                                                const double rneed_x = std::fabs(
                                                    (static_cast<double>(st.R_xdiag.at(x0)) / u_x) - 0.5);
                                                const double score =
                                                        (0.6 * rneed_c) + (0.2 * rneed_d) + (0.2 * rneed_x);
                                                if (score > best_score) {
                                                    best_score = score;
                                                    c_pick = c0;
                                                    found = true;
                                            ...
                                            for (int vv = 0; vv < 2 && !adopted_step; ++vv) {
                                                ...
                                                const std::size_t d0 = ::crsce::decompress::detail::calc_d(
                                                    boundary, c_pick);
                                                const std::size_t x0 = ::crsce::decompress::detail::calc_x(
                                                    boundary, c_pick);
                                                ...
                                                if (st_try.U_row.at(boundary) == 0 || st_try.U_row.at(boundary) <
                                                    ...
                                                    if (st_try.U_row.at(boundary) == 0) {
                                                        ...
                                                        const std::size_t check_rows = std::min<std::size_t>(
                                                            valid_now + 1, S);

                                                        if (check_rows > 0) {
                                                            const auto t0vv = std::chrono::steady_clock::now();
                                                            const bool okvv = verifier_try.verify_rows(
                                                                c_try, lh, check_rows);
                                            ...
                                            if (steps > 0) {
                                                ...
                                                if (st.U_row.at(boundary) == 0) {
                                                    ...
                                                    if (prefix_rows2 > 0) {
                                                        ...
                                                        const bool okvv2 = verifier.verify_rows(
                                                            csm_out, lh, prefix_rows2);
                                ...
                                if (gprog == 0 && plateau_flip_attempts >= 3) {
                                    ...
                                    if (polish_shakes_remaining > 0 && ph == (dampers.size() - 1)) {
                                        ...
                                        const double jitter = std::min(
                                            0.012,
                                            step * static_cast<double>(kPolishShakes - polish_shakes_remaining + 1));
                                        std::uniform_real_distribution<double> delta(-jitter, jitter);
                                        for (std::size_t r0 = 0; r0 < S; ++r0) {
                                            for (std::size_t c0 = 0; c0 < S; ++c0) {
                                                if (csm_out.is_locked(r0, c0)) { continue; }
                ...
                if (!det.solved()) {
                    ...
                    if (boundary < S) {
                        ...
                        std::vector<std::pair<double, std::size_t> > cand_cols;
                        cand_cols.reserve(S);
                        for (std::size_t c0 = 0; c0 < S; ++c0) {
                            if (csm_out.is_locked(boundary, c0)) {
                                continue;
                            ...
                            for (std::size_t a = 0; a + 1 < K2 && !adopted_prefix; ++a) {
                                for (std::size_t b = a + 1; b < K2 && !adopted_prefix; ++b) {
                                    ...
                                    for (int mask = 0; mask < 4 && !adopted_prefix; ++mask) {
                                        ...
                                        const std::size_t d_i = (c_i >= boundary)
                                                                    ? (c_i - boundary)
                                                                    : (c_i + S - boundary);
                                        ...
                                        const std::size_t d_j = (c_j >= boundary)
                                                                    ? (c_j - boundary)
                                                                    : (c_j + S - boundary);
                                        ...
                                        if (st_try.U_row.at(boundary) == 0) {
                                            ...
                                            if (constexpr RowHashVerifier verifier_try;
                                                check_rows > 0 && verifier_try.verify_rows(c_try, lh, check_rows)) {
                                                csm_out = c_try;
                                                st = st_try;
                                                adopted_prefix = true;
                                                std::size_t sumU2 = 0;
