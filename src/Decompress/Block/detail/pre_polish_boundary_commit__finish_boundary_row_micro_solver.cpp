...
namespace crsce::decompress::detail {
    ...
    bool finish_boundary_row_micro_solver(Csm &csm_out, // NOLINT(misc-use-internal-linkage,readability-function-size)
        ...
        // Gate by near-threshold unknowns (default ~40%; overridable via CRSCE_FOCUS_NEAR_THRESH)
        auto near_thresh = static_cast<std::uint16_t>(((S * 2U) + 4U) / 5U);
            ...
            if (remaining == 0) {
                    ...
                    if (dp_ok) {
                        ...
                        // Swap variants: swap early chosen indices with top not-chosen ambiguous candidates (within window)
                        {
                            ...
                            if (!amb_pool.empty()) {
                                ...
                                const std::size_t pos_limit = std::min<std::size_t>(chosen.size(), 16);
                ...
                if ((!dp_gate_ok || !dp_succeeded)) {
                    ...
                    if (!amb.empty()) {
                        ...
                        amb_cap = std::min<std::size_t>(amb_cap, 128);
