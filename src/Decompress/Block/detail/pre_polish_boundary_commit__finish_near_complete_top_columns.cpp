...
namespace crsce::decompress::detail {
    ...
    /**
     * @name finish_near_complete_top_columns
     * @brief Attempt completion for top-N near-complete columns using top-K ambiguous cells.
     ...
     * @param top_n_cols Number of near-complete columns to consider.
     * @param top_k_cells Number of cells to try per column.
     * @return bool True if any column completion was adopted; otherwise false.
     */
        ...
        for (std::size_t ci = 0; ci < N; ++ci) {
            ...
            for (std::size_t i = 0; i < K; ++i) {
                ...
                for (int vv = 0; vv < 2; ++vv) {
                    ...
                    if (assume_one) { c_try.set(r, c); } else { c_try.clear(r, c); }
                    c_try.lock(r, c);
