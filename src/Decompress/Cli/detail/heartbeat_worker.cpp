            ...
            if (snap_opt) {
                ...
                switch (s.phase) {
                    ...
                    case ::crsce::decompress::BlockSolveSnapshot::Phase::radditzSift: {
                        // Hybrid summary (residual-driven)
                        oss << ",hyb_passes=" << static_cast<std::uint64_t>(s.hyb_passes_last)
                            << ",hyb_swaps_total=" << static_cast<std::uint64_t>(s.hyb_swaps_total)
                            << ",hyb_acc/att=" << static_cast<std::uint64_t>(s.hyb_rect_accepts_total)
                            << '/' << static_cast<std::uint64_t>(s.hyb_rect_attempts_total);
                        break;
                    }
