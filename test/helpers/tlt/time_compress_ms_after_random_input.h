...
namespace crsce::testhelpers::tlt {
    /**
     * @name time_compress_ms_after_random_input
     * @brief Create random input for N blocks, run compress and time it; then decompress and verify.
     ...
     * @return Pair<roundtrip_ok, compress_ms>.
     */
    inline std::pair<bool, std::uint64_t> time_compress_ms_after_random_input(const std::string &dir,
        ...
        const int rc = run_compress_cli(in_path, cr_path);
        const auto t1 = std::chrono::steady_clock::now();
        if (rc != 0 || !fs::exists(cr_path)) {
            remove_file(in_path); remove_file(cr_path); remove_file(out_path);
            return {false, 0};
        }
