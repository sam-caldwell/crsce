/**
 * @file test_runner_alternating10_cli_integration.cpp
 * @brief Integration test for the TestRunnerAlternating10 pipeline via direct run().
 */
...
TEST(TestRunnerAlternating10, CliEndToEndBestEffort) {
    const fs::path old = fs::current_path();
    fs::current_path(TEST_BINARY_DIR);
    const fs::path out_dir = fs::path(tmp_dir()) / "tralt10_cli";
    fs::create_directories(out_dir);

    // Run best-effort: if any case fails, continue to verify successful pairs.
    try {
        (void)crsce::testrunner_alternating10::cli::run(out_dir);
    ...
    for (const auto &suf : suffixes) {
        const auto in_prefix = std::string("alternating10_input_") + suf + "_";
        const auto dx_prefix = std::string("alternating10_reconstructed_") + suf + "_";
