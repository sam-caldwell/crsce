/**
 * @file test_runner_zeroes_cli_integration.cpp
 * @brief Integration test for the TestRunnerZeroes pipeline via direct run().
 */
...
TEST(TestRunnerZeroes, CliEndToEnd) {
    ...
    const fs::path out_dir = fs::path(tmp_dir()) / "trzeroes_cli";
    ...
    for (const auto &suf : suffixes) {
        const auto in_glob = out_dir / ("zeroes_input_" + suf + "_");
        const auto dx_glob = out_dir / ("zeroes_reconstructed_" + suf + "_");
