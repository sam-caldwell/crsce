/**
 * @file clang_plugin_odph_missing_header_copyright_error_test.cpp
 * @brief Missing copyright markers in header doc should fail.
 */
 ...
 * @name ClangPluginODPH.MissingHeaderCopyrightFails
...
TEST(ClangPluginODPH, MissingHeaderCopyrightFails) {
    std::string log;
    ...
    const auto fixture =
            ...
            "missing_header_copyright/include/MissingHeaderCopyright.h";
    ASSERT_TRUE(std::filesystem::exists(fixture));

    std::string out;
    const int code = clang_compile_with_plugin(fixture, lib, out);
    EXPECT_NE(code, 0) << "Expected failure for missing copyright markers\n"
                     << out;
