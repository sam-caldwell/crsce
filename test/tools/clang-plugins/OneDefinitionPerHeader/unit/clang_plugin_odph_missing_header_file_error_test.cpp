/**
 * @file clang_plugin_odph_missing_header_file_error_test.cpp
 * @brief Missing @file in header doc should fail.
 */
 ...
 * @name ClangPluginODPH.MissingHeaderFileTagFails
...
TEST(ClangPluginODPH, MissingHeaderFileTagFails) {
    std::string log;
    ...
    const auto fixture =
            ...
            "missing_header_file/include/MissingHeaderFile.h";
    ASSERT_TRUE(std::filesystem::exists(fixture));

    std::string out;
    const int code = clang_compile_with_plugin(fixture, lib, out);
    EXPECT_NE(code, 0) << "Expected failure for missing @file tag in header doc\n"
                     << out;
