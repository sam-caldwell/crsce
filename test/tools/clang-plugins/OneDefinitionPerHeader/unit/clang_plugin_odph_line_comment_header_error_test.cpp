/**
 * @file clang_plugin_odph_line_comment_header_error_test.cpp
 * @brief Header doc uses // comment at line 1; should fail.
 */
 ...
 * @name ClangPluginODPH.LineCommentHeaderFails
...
TEST(ClangPluginODPH, LineCommentHeaderFails) {
    std::string log;
    ...
    const auto fixture =
            ...
            "line_comment_header/include/LineCommentHeader.h";
    ASSERT_TRUE(std::filesystem::exists(fixture));

    std::string out;
    const int code = clang_compile_with_plugin(fixture, lib, out);
    EXPECT_NE(code, 0) << "Expected failure for //-style header doc at line 1\n"
                     << out;
