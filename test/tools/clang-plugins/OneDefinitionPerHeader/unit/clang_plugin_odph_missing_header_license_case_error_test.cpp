/**
 * @file clang_plugin_odph_missing_header_license_case_error_test.cpp
 * @brief Header doc uses LICENSE.TXT (wrong case); should fail.
 */
 ...
 * @name ClangPluginODPH.MissingHeaderLicenseCaseFails
...
TEST(ClangPluginODPH, MissingHeaderLicenseCaseFails) {
    std::string log;
    ...
    const auto fixture =
            ...
            "missing_header_license_case/include/MissingHeaderLicenseCase.h";
    ASSERT_TRUE(std::filesystem::exists(fixture));

    std::string out;
    const int code = clang_compile_with_plugin(fixture, lib, out);
    EXPECT_NE(code, 0) << "Expected failure for wrong-case LICENSE.TXT\n" << out;
