/**
 * @file clang_plugin_odph_missing_header_at_line1_error_test.cpp
 * @brief Header doc must start at line 1 and be block-style.
 */
 ...
 * @name ClangPluginODPH.MissingHeaderAtLine1Fails
...
TEST(ClangPluginODPH, MissingHeaderAtLine1Fails) {
    std::string log;
    ...
    const auto fixture =
            ...
            "missing_header_at_line1/include/MissingHeaderAtTop.h";
    ASSERT_TRUE(std::filesystem::exists(fixture));

    std::string out;
    const int code = clang_compile_with_plugin(fixture, lib, out);
    EXPECT_NE(code, 0) << "Expected failure for header doc not at line 1\n"
                     << out;
