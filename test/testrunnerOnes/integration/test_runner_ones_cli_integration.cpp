...
TEST(TestRunnerOnes, CliEndToEnd) {
    ...
    const fs::path out_dir = fs::path(tmp_dir()) / "trones_cli";
    ...
    for (const auto &suf : suffixes) {
        const auto in_glob = out_dir / ("ones_input_" + suf + "_");
        const auto dx_glob = out_dir / ("ones_reconstructed_" + suf + "_");
