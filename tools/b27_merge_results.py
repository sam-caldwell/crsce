#!/usr/bin/env python3
"""
B.27 Merge Results — Combine per-worker output from b27_seed_search.py.

Reads all b27_results_w{N}.json files from the output directory, merges into a
...
Usage:
    ...
    python3 tools/b27_merge_results.py --out-dir tools/b27_results --workers 4 --out merged.json

Options:
    --out-dir DIR   Directory containing per-worker result files (default: tools/b27_results/)
    --workers N     Number of workers to merge (default: 4)
    --out FILE      Write merged results to FILE (default: <out-dir>/b27_merged.json)
...
BASELINE_SEED5_LABEL = "CRSCLTP5"
BASELINE_SEED6_LABEL = "CRSCLTP6"

# B.26c winner (seeds 1+2) for reference
B26C_DEPTH = 91_090
...
def load_worker_results(out_dir: pathlib.Path, workers: int) -> tuple[list[dict], list[int]]:
    ...
    for w in range(workers):
        path = out_dir / f"b27_results_w{w}.json"
        if not path.exists():
            missing.append(w)
            continue
        try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Merge B.27 per-worker results")
    ...
    parser.add_argument("--out", default=None,
                        help="Output file path (default: <out-dir>/b27_merged.json)")
    ...
    out_path = pathlib.Path(args.out) if args.out else out_dir / "b27_merged.json"

    print(f"B.27 Merge Results")
    ...
    if missing:
        ...
        print(f"  Re-run: python3 tools/b27_seed_search.py "
              f"--worker N --workers {args.workers}")

    if not all_results:
        sys.exit("\nNo results to merge.")

    # Deduplicate by (seed5, seed6) key, keeping the higher depth
    seen: dict[str, dict] = {}
    ...
    if best:
        ...
        print(f"  Global best:       seed5={best['seed5_label']} seed6={best['seed6_label']} "
    ...
    for i, r in enumerate(deduped[:top_n]):
        marker = ""
        if r.get("seed5_label") == BASELINE_SEED5_LABEL \
                and r.get("seed6_label") == BASELINE_SEED6_LABEL:
            marker = " [baseline]"
        elif r["depth"] > BASELINE_DEPTH:
            marker = " [beats baseline]"
        print(f"  {i+1:3d}. seed5={r['seed5_label']:<16s} seed6={r['seed6_label']:<16s} "
              f"depth={r['depth']}{marker}")

    # Write merged output
    merged = {
        "search": "B.27 full-byte joint seed5+seed6 search",
        "candidate_range": "0x00..0xFF (256 per seed axis)",
        "fixed_seeds": {
            "seed1": "CRSCLTPV", "seed2": "CRSCLTPP",
            "seed3": "CRSCLTP3", "seed4": "CRSCLTP4",
        },
        "total_pairs_expected": total_expected,
        "total_pairs_completed": len(deduped),
        "coverage_pct": round(coverage, 2),
        "baseline": {
            "seed5_label": BASELINE_SEED5_LABEL,
            "seed6_label": BASELINE_SEED6_LABEL,
            "depth": BASELINE_DEPTH,
        },
        "best_found": {
            "seed5": best["seed5"],
            "seed6": best["seed6"],
            "seed5_label": best.get("seed5_label", ""),
            "seed6_label": best.get("seed6_label", ""),
            "depth": best["depth"],
    ...
    if best and best["depth"] > BASELINE_DEPTH:
        print(f"\nUpdate LtpTable.cpp:")
        print(f"  constexpr std::uint64_t kSeed5 = {best['seed5']}ULL;  "
              f"// {best.get('seed5_label', '')}")
        print(f"  constexpr std::uint64_t kSeed6 = {best['seed6']}ULL;  "
              f"// {best.get('seed6_label', '')}")
