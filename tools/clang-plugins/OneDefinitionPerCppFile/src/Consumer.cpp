/**
 * @file Consumer.cpp
 * @brief Definition of AST consumer for ODPCPP.
 */
...
void Consumer::HandleTranslationUnit(ASTContext &Context) {
  // Bind AST context and source manager for downstream operations
  Ctx_->setASTContext(Context);
  auto &SM = Context.getSourceManager();
  auto FID = SM.getMainFileID();
  auto Path = SM.getFilename(SM.getLocForStartOfFile(FID));
  llvm::errs() << "ODPCPP: BEGIN TU " << Path << "\n"; llvm::errs().flush();
  Finder_->matchAST(Context);
  Ctx_->finalizeTU();
  llvm::errs() << "ODPCPP: END TU " << Path << "\n"; llvm::errs().flush();
}
