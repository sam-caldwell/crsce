 ...
 * @brief LateralHash class declaration for storing and verifying per-row SHA-256 digests.
 *
 * Stores s SHA-256 digests (one per CSM row).  Provides compute() to hash a
...
namespace crsce::common {
     ...
     * @class LateralHash
     * @name LateralHash
     * @brief Stores s SHA-256 digests (one per CSM row) and provides hash computation and verification.
    ...
    private:
        ...
        /**
         * @name digests_
         * @brief Vector of s SHA-256 digests, one per row.
         */
