...
We can add guarded env flags like:
- CRSCE_GOBP_RESTARTS=N
- CRSCE_GOBP_PERTURB=epsilon (small random jitter)
- Targeted backtracking: already enabled (CRSCE_BACKTRACK=1), but we can expand selection to “top‑K” most‑uncertain cells or
  most‑constrained rows/cols under a strict iteration/time budget.
- Prefix‑LH gating: as rows become fully solved from the top, verify the chain prefix; if it fails, trigger a localized
  backtrack/perturbation to escape an inconsistent basin.
- Symmetry breaking: prioritize DE sweeps on rows/cols with highest completion, forcing tie‑breaks to avoid flat plateaus.
